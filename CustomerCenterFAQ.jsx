...
function Badge({ spec }) {
  return (
    <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, lineHeight: 1, padding: "4px 7px", borderRadius: 5, marginRight: 8, verticalAlign: "1px", letterSpacing: "0.03em", background: spec ? "#eef1ff" : "#f0f1f3", color: spec ? T.primary : T.placeholder }}>
      {spec ? "SPEC" : "DUMMY"}
    </span>
  );
}
...
const SPEC = [
...

  {
    id: "scroll", kind: "spec",
    tag: "스크롤",
    question: "항목이 열릴 때 자동 스크롤은 어떻게 동작하나요?",
    answer: (
      <>
        <P>항목을 펼치면 해당 헤더가 sticky GNB 아래 눈높이로 자동 이동합니다. (이 FAQ 아코디언 리스트 동작)</P>
        <SpecTable rows={[
          ["메서드", "el.scrollIntoView({ behavior: 'smooth', block: 'start' })"],
          ["오프셋", "scroll-margin-top: 88px  /* GNB 64 + 여백 24 */"],
          ["실행 시점", "펼침 트랜지션 종료 후 = 300ms + 30ms"],
          ["초기 렌더", "스킵 (기본 열림 항목은 자동 스크롤 안 함)"],
        ]} />
        <SubHead>개발</SubHead>
        <Bullets items={[
          "오프셋은 CSS scroll-margin-top으로 처리 → sticky 헤더에 가리지 않음,",
          "단일 펼침은 이전 항목이 닫히며 위치가 바뀌므로 트랜지션 종료 후 스크롤해야 어긋나지 않음,",
          "JS는 scrollIntoView 호출만. useEffect([isOpen]) + mounted ref로 초기 렌더 스킵,",
        ]} />
        <Note>화면 정중앙 정렬을 원하면 <HL>block: 'center'</HL>로 변경. 멈추는 높이는 scroll-margin-top 값으로 조정.</Note>
      </>
    ),
  },
  {
    id: "hover", kind: "spec",
    tag: "호버",
    question: "목록에 마우스를 올리면 어떻게 딤되나요?",
    answer: (
      <>
        <P>목록 행에 hover 시 배경 딤 + 셰브론 색 변화가 적용됩니다.</P>
        <SpecTable rows={[
          ["행 배경", "#0000000d  /* 5% 블랙 = surface/section/divider-thin-transparent */"],
          ["셰브론 color", "#545b69 → #2a2e36"],
          ["transition", "background, color · 140ms ease"],
          ["행 radius", "8px  /* 하이라이트 라운드 */"],
        ]} />
        <Note>딤 농도는 토큰값(<HL>T.dimThin</HL>) 교체로 조정. 예: #00000014(8%)면 더 또렷. 펼침/스크롤 모션과 독립적으로 동작.</Note>
      </>
    ),
  },
